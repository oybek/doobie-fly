...
import io.github.oybek.kraken.migration.model.{Commit, Migration}
import io.github.oybek.kraken.migration.model.{Commit, Migration}

package object migration {

  implicit val showFragment: Show[Fragment] =
    Show.show((x: Fragment) => x.update.sql)

  def migrate[F[_]: Bracket[*[_], Throwable]](
  ...
  private def cases[F[_]: Bracket[*[_], Throwable]](
    log: Option[String => F[Unit]]
  )(commit: Option[Commit],
    migration: Option[Migration])(implicit tx: Transactor[F]): F[Unit] =
